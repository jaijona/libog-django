...
def guardar_promedios_stripchat():
    try:
        ...
        # Mapeo de géneros del modelo al primaryTag de Stripchat
        genero_a_primaryTag = {
            "Mujer": "girls",
            "Hombre": "men",
            "Trans": "trans",
        ...
        for studio_id in studio_ids:
            ...
            for modelo in modelos_registrados:
                genero = modelo.genero  # Debe ser Mujer, Hombre, Trans, Pareja
                if genero in genero_a_primaryTag and modelo.usuario_strip:
                    modelos_por_genero.setdefault(genero, []).append(modelo)

            for genero, modelos in modelos_por_genero.items():
                primary_tag = genero_a_primaryTag[genero]
                offset = 0
                posicion_global = 0
                encontrados = []
                ...
                if not encontrados:
                    print(f"⚠️ No se encontraron modelos para género {genero}")
                    continue

                usernames_deseados = {
                    m.usuario_strip.strip().lower(): m for m in modelos
                }
